...
module.exports = {
  ...
  onStart: async function ({ message, event, args, api }) {
    ...
    try {
      const { data } = await axios.get(`${await aApi()}/nazrul/xl?prompt=${encodeURIComponent(args.join(" "))}`);
  ...
  onReply: async function ({ message, event, Reply, args }) {
    ...
    message.reply({
      ...
      attachment: await getStreamFromURL(img, `xl_${choice}.jpeg`)
    });
  }
};
...
async function createGridImage(urls) {
  ...
  const filePath = path.join(__dirname, `xl_grid_${Date.now()}.jpeg`);
